...
$(document).ready(function () {
  ...
  $('input[type="text"]').each(function () {
    ...
    $(this).on("input", () => {
      ...
      timer = setTimeout(() => {
        $(this).val() === ""
          ? isBlank(`error-${elemID}`, elemID)
          : !regEx.test($(this).val())
          ? notValid("text", `error-${elemID}`, elemID)
          : isValid(`error-${elemID}`, elemID);
      }, delay);
    });
  });
  ...
  $('input[type="email"]').each(function () {
    ...
    $(this).on("input", () => {
      ...
      timer = setTimeout(() => {
        $(this).val() === ""
          ? isBlank(`error-${elemID}`, elemID)
          : !emailRegEx.test($(this).val())
          ? notValid("email", `error-${elemID}`, elemID)
          : isValid(`error-${elemID}`, elemID);
      }, delay);
    });
  });
  ...
  $("input[type='radio']").on("click", () => {
    clearTimeout(timer);
    timer = setTimeout(() => {
      !$('input[name="option"]:checked').length > 0
        ? notValid("radio", "error-radio")
        : isValid("error-radio", "radio");
    }, delay);
  });
  ...
  $("textarea").on("input", () => {
    ...
    timer = setTimeout(() => {
      $("textarea").val() === ""
        ? isBlank("error-msg", "message")
        : isValid("error-msg", "message");
    }, delay);
  });
  ...
  $("input[type='checkbox']").on("click", () => {
    clearTimeout(timer);
    timer = setTimeout(() => {
      !$('input[name="consent"]:checked').length > 0
        ? notValid("checkbox", "error-consent", "consent")
        : isValid("error-consent", "consent");
    }, delay);
  });
  ...
  function validateInput(inputId, errorId, type) {
    ...
    switch (type) {
      case "text":
        newID.val() == ""
          ? isBlank(`error-${errorId}`, inputId)
          : !regEx.test(newID.val())
          ? notValid("text", `error-${elemID}`, elemID)
          : isValid(`error-${errorId}`, inputId);
        break;
      case "email":
        newID.val() == ""
          ? isBlank(`error-${errorId}`, inputId)
          : !emailRegEx.test($(newID).val())
          ? notValid("email", `error-${errorId}`, inputId)
          : isValid(`error-${errorId}`, inputId);
        break;
      case "radio":
        !$('input[name="option"]:checked').length > 0
          ? notValid("radio", `error-${errorId}`)
          : isValid(`error-${errorId}`, inputId);
        break;
      case "checkbox":
        !$('input[name="consent"]:checked').length > 0
          ? notValid("checkbox", `error-${errorId}`, inputId)
          : isValid(`error-${errorId}`, inputId);
        break;
      case "message":
        newID.val() == ""
  ...
  $(".btn-submit").click(function validateInputs(event) {
    ...
    $(".error-message").each(function () {
      $(this).attr("aria-hidden") == "false" ? (errorsExist = true) : false; // Exit the loop early if an error is found
    });
